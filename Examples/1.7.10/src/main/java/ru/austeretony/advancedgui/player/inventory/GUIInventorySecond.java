...
import ru.austeretony.advancedgui.player.inventory.slot.InventorySlotRenderer;
import ru.austeretony.advancedgui.player.inventory.sorters.ArmorSorter;
import ru.austeretony.advancedgui.player.inventory.sorters.CommonSorter;
...
public class GUIInventorySecond extends AdvancedGUIContainer {
	
	private static final ResourceLocation TEXTURE_INVENTORY = new ResourceLocation(AdvancedGUIMain.MODID, "textures/gui/inventory/inventorySecond.png");
	...
	/*
	 * Кастомный рендерер для слотов, позволит управлять отрисовкой.
	 */
	...
	COMMON_SORTER = new CommonSorter(),  
	...
	//Кнопки, функционал реализуется в методе handleButtonPress().
	//Кнопки используются для сортировки инвентаря по секциям (броня, оружие и т.д.)
    private GUIButton 
    commonSorter = new GUIButton(), 
	...
	public GUIInventorySecond(EntityPlayer player, PlayerInventory inventory) {
    ...
    private void initInventory() {
    	...
    	/*
    	 * Для слотов инвентаря используется кастомный рендерер, увеличивающий ширину слотов.
    	 * Теперь в одной строке будет только один столбец (1 слот, не 5), кол-во строк прежднее.
    	 */
    	 ...
    	 * 6 - кол-во слотов отображаемых по вертикали. В итоге 10 слотов будут отображены в области 10 * 1 (10 слотов видны, 140 скрыты).
    	 */   
    	this.inventoryFramework = new GUIFramework(GUIPosition.CUSTOM, this.inventory, 13, 162, 10, 1);//Инициализация, использование координат из ГПИ.
    	...
    	//Создание рендерера слотов.
        this.slotRenderer = new InventorySlotRenderer(88, 16);
        
        //Регистрация рендерера для фрейма инвентаря.
        this.inventoryFramework.initSlotRenderer(this.slotRenderer);
        
        //Коррекция расстояния между слотами (1 пиксель по вертикали, вместо 2 по умалчанию, по горизонтали не важно (1 столбец)).
        this.inventoryFramework.setSlotDistance(0, 1);
    	 ...
    	 */
    	this.slider = new GUISlider(this.scroller, 351, 50, 6, 170, 1.0F);
    	...
    	this.commonSorter.setToggled(true);
    	
    	//Установка текстур для кнопок.
    	this.commonSorter.setTexture(new ResourceLocation(AdvancedGUIMain.MODID, "textures/gui/inventory/buttons/main.png"), 15, 15);
    	...
    	this.commonSorter.setPopupText("guiButton.mainSection");
    	...
    	this.sortersPanel.addButton(this.commonSorter);
    	...
    	this.updateFramework(this.inventoryFramework, this.COMMON_SORTER);//Загрузка сначала не пустых слотов, затем пустых.
    }
    
    protected void handleButtonPress(GUIFramework framework, GUIButton button) {
    	
    	if (button == this.commonSorter) {//Если нажата кнопка "инвентарь"
    		
    		this.updateFramework(this.inventoryFramework, this.COMMON_SORTER);//Загрузка оригинального содержимого.
    		
    		this.commonSorter.setToggled(true);//"Зажимаем" кнопку (подсвечивание).
    	}
    	
    	else {
    		
    		this.commonSorter.setToggled(false);//Если нажата другая кнопка - "отжимаем" эту.
    	}
