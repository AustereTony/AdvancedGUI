		...
		switch (this.inventoryId) {
			...
			case 0:
				...
				if (player.openContainer instanceof ContainerInventory) {
					...
					player.entityDropItem(((ContainerInventory) player.openContainer).inventory.getStackInSlot(this.slotIndex), 1.0F).setDefaultPickupDelay();
					((ContainerInventory) player.openContainer).inventory.setInventorySlotContents(this.slotIndex, ItemStack.EMPTY.EMPTY);
				}
