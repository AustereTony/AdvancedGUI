...
public class ActionEquip extends GUIPresetAction {
	...
	public boolean isValidAction(IInventory inventory, Slot slot, EntityPlayer player) {		
		...
		boolean 
		isWeapon = slot.getStack().getItem() instanceof ItemSword || slot.getStack().getItem() instanceof ItemBow || 
		slot.getStack().getItem() instanceof ItemTool || slot.getStack().getItem() instanceof ItemFood || slot.getStack().getItem() instanceof ItemPotion,
